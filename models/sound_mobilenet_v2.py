...
class MobileNetV2(nn.Module):
    ...
    def network_name(self):
        name = 'sound_mobilenet_v2-{}'.format(self.depth)
        return name

def sound_mobilenet_v2(num_classes, input_channels, dropout, imagenet_pretrained=True, **kwargs):
    """
    ...
    """
    model = MobileNetV2(num_classes=num_classes, input_channels=input_channels, dropout=dropout)
    if imagenet_pretrained:
        ...
        if input_channels != 3:  # convert the RGB model to others, like flow
            ...
            state_dict['features.0.0.weight'] = value.mean(dim=1, keepdim=True).expand(new_shape).contiguous()
...
if __name__ == "__main__":
    sound_mobilenet_v2(239, 1, 0.5, True)
